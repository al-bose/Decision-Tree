...
# Reads the data from CSV files, each attribute column can be obtained via its name, e.g., y = data['y']f
...
def compute_info_gain(data, att_name, y_name):
    ...
    attribute_counts = np.zeros([8,9])
    a = data.drop(data.columns.difference([att_name]), axis = 1).get_values()
    b = data.drop(data.columns.difference([y_name]), axis = 1).get_values()
    
    for i in range(data.shape[0]):
        attribute_counts[int(b[i][0])][int(a[i][0])] += 1
    ...
    for i in range(1,8):
        np.put(attr_0, i, attribute_counts[i][0])
        np.put(attr_1, i, attribute_counts[i][1])
        np.put(attr_2, i, attribute_counts[i][2])
        np.put(attr_3, i, attribute_counts[i][3])
        np.put(attr_4, i, attribute_counts[i][4])
        np.put(attr_5, i, attribute_counts[i][5])
        np.put(attr_6, i, attribute_counts[i][6])
        np.put(attr_7, i, attribute_counts[i][7])
        np.put(attr_8, i, attribute_counts[i][8])

    info.append(compute_entropy(attr_0))
    info.append(compute_entropy(attr_1))
    info.append(compute_entropy(attr_2))
    info.append(compute_entropy(attr_3))
    info.append(compute_entropy(attr_4))
    info.append(compute_entropy(attr_5))
    info.append(compute_entropy(attr_6))
    info.append(compute_entropy(attr_7))
    info.append(compute_entropy(attr_8))
    for j in range(len(counts)):
        if(info[j] != 0):
            conditional_entropy += info[j] * (counts[j]/total_counts)
...
def comput_gain_ratio(data, att_name, y_name):
    ...
    for i in range(len(counts)):
        ratio = counts[i]/total_counts
        att_info -= ratio * (np.log2(ratio))
